...
class LearningSection {
	constructor(element) {
		...
		let learningClassesElements = element.querySelectorAll('.learning__class');//return the descendants of the element 
        this.condenseElement = element.querySelector('#learning-condensed-button');//get element button 
	...
	stopRecording() {
		this.wiresLeft.dehighlight()//wire;
	}
	
	ledOn(id) {
		this.wiresRight.dehighlight();//bulb
		this.wiresRight.highlight(id);
	}

	getMaxIndex(array) {//get index of  the element have highest confidences
		let max = array[0];
		let maxIndex = 0;
	...
	// use function
	setConfidences(confidences) {//learning part/ update automatically
		let maxIndex = this.getMaxIndex(confidences);
		let maxValue = confidences[maxIndex];
		// if (maxValue > 0.5 && this.currentIndex !== maxIndex) {
		if (maxValue > 0.5) {//if max value >0.5 then use function outputSection trigger the id 
			this.currentIndex = maxIndex;
			let id = GLOBALS.classNames[this.currentIndex];//get the class name of highest value 
			this.ledOn(id);//UI highlight bulb on right wire.

			GLOBALS.outputSection.trigger(id);//implemented the highest confidence index classes
		}

		for (let index = 0; index < 3; index += 1) {// loop throught all classes find the highest confidences and highlight it
